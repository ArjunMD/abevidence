...
# ---------------- Tools: medication adverse effects ----------------

@st.cache_data(ttl=60 * 60 * 24, show_spinner=False)
def medication_adverse_effects(medication: str, side_effect: str = "") -> dict:
    """AI lookup for a drug's adverse effects. Two modes:

    - No `side_effect`: return the most common / clinically important adverse
      effects of `medication` (mode "list").
    - `side_effect` given: assess whether `medication` has been reported to
      cause that specific effect (mode "check").

    Returns for "list": {"mode": "list", "medication": str,
        "effects": [{"effect": str, "detail": str}]}
    Returns for "check": {"mode": "check", "medication": str, "side_effect": str,
        "verdict": "yes"|"possible"|"no", "explanation": str, "notes": [str]}
    Cached for a day so re-running the same query doesn't re-bill."""
    medication = (medication or "").strip()
    side_effect = (side_effect or "").strip()
    if not medication:
        return {}
    ...
    if not side_effect:
        instructions = (
            "You are a clinical pharmacology reference for a hospital-medicine "
            "physician. Given a medication (generic or brand name), list its most "
            "common and most clinically important adverse effects.\n"
            "Rules:\n"
            "- 'medication': the drug's generic name (add the queried brand in "
            "parentheses if the user gave a brand).\n"
            "- 'effects': 6-12 items, most common or most serious first. For each:\n"
            "   * 'effect': the adverse effect, named concisely.\n"
            "   * 'detail': ≤15 words — frequency (common/uncommon/rare), whether it "
            "is serious/black-box, and any key nuance. No boilerplate.\n"
            "- Prioritize what actually changes management (the famous, board-relevant, "
            "and dangerous effects). Include black-box warnings.\n"
            "- If the drug name is unrecognized or ambiguous, return an empty "
            "'effects' list and name the problem in 'medication'.\n"
            'Return ONLY JSON: {"medication": "...", "effects": '
            '[{"effect": "...", "detail": "..."}]}'
        )
        user_input = f"Medication: {medication}\n\nJSON:"
        mode = "list"
    else:
        instructions = (
            "You are a clinical pharmacology reference for a hospital-medicine "
            "physician. Given a medication and a specific adverse effect, assess "
            "whether that medication has been reported to cause that effect.\n"
            "Rules:\n"
            "- 'medication': the drug's generic name (add the queried brand in "
            "parentheses if the user gave a brand).\n"
            "- 'side_effect': echo the effect being asked about.\n"
            "- 'verdict': 'yes' if it is a recognized/reported adverse effect, "
            "'possible' if there are only rare case reports or a plausible "
            "mechanism without established association, 'no' if it is not a known "
            "association.\n"
            "- 'explanation': JUST the answer, in 1-2 sentences — whether it is "
            "reported and how established or frequent the link is (e.g. common, "
            "well-recognized, uncommon, rare, only case reports or postmarketing "
            "reports), and the typical form it takes if that is the key fact. Do "
            "NOT lecture: no mechanism speculation, no differential of other "
            "causes, no workup, no monitoring or management advice. Tie to real "
            "pharmacology; do not invent associations.\n"
            "- If the drug or effect is unrecognized, set verdict 'no' and say so.\n"
            'Return ONLY JSON: {"medication": "...", "side_effect": "...", '
            '"verdict": "...", "explanation": "..."}'
        )
    ...
    if mode == "list":
        ...
        for e in data.get("effects") or []:
            if not isinstance(e, dict):
                continue
            name = str(e.get("effect") or "").strip()
            if not name:
                continue
            effects.append({"effect": name, "detail": str(e.get("detail") or "").strip()})
        return {
            "mode": "list",
            "medication": str(data.get("medication") or medication).strip(),
            "effects": effects,
        }

    verdict = str(data.get("verdict") or "").strip().lower()
    if verdict not in ("yes", "possible", "no"):
        verdict = "possible"
    return {
        "mode": "check",
        "medication": str(data.get("medication") or medication).strip(),
        "side_effect": str(data.get("side_effect") or side_effect).strip(),
        "verdict": verdict,
